...
void game1_start()
{
  ...
  // pwrmgr_setState(PWR_ACTIVE_DISPLAY, PWR_STATE_BUSY);
	...
	buttons_setFuncs(btnLeft, btnRight, btnExit);
...
display_t game1_draw()
{
	...
	LOOP(BLOCK_COLS, x)
	{
		LOOP(BLOCK_ROWS, y)
		{
			...
			{
				buzzer_buzz(100, TONE_2KHZ, VOL_UI, 0, NULL);
	...
	if(ballX < 0)
  {
	...
	if(!gameEnded && ballY >= FRAME_HEIGHT - PLATFORM_HEIGHT && ballY < 240 && ballX >= platformX && ballX <= platformX + PLATFORM_WIDTH)
	{
		...
		buzzer_buzz(200, TONE_5KHZ, VOL_UI, 0, NULL);
	...
	if(!gameEnded && !platformCollision && (ballY > FRAME_HEIGHT - 2 || blockCollide))
	{
		if(ballY > 240)
		{
			buzzer_buzz(200, TONE_2_5KHZ, VOL_UI, 0, NULL);
			ball.y = 0;
		}
		else if(!blockCollide)
		{
			buzzer_buzz(200, TONE_2KHZ, VOL_UI, 0, NULL);
	...
	if(score >= BLOCK_COUNT)
		draw_string((STR_WIN), false, 50, 32);
